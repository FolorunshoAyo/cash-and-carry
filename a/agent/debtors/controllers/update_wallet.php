    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($wid) || empty($daily_payment) || empty($days) || empty($pwd)){
            ...
            if(password_verify($pwd, $agent_passcode)){
                ...
                $sql_check_for_existing_savings = $db->query("SELECT * from debtor_savings WHERE
                ...
                if($sql_check_for_existing_savings->num_rows > 0){
                    $sql_check_latest_savings = $db->query("SELECT * FROM debtor_savings WHERE wallet_id='$wid' ORDER BY savings_id DESC"); 
                    ...
                    $sql_insert_savings = $db->query("INSERT INTO debtor_savings (wallet_id, amount, savings_days, start_date, end_date) VALUES ('$wid', '$calculated_amount', '$days', '$new_savings_start', '$new_savings_end')");
                }else{
                    $new_savings_end = date("Y-m-d", strtotime($today . " + " . ($days - 1) . " days"));

                    // IF THERE ARE NO UPFRONT PAYMENT
                    $sql_insert_savings = $db->query("INSERT INTO debtor_savings (wallet_id, amount, savings_days, start_date, end_date) VALUES ('$wid', '$calculated_amount', '$days', '$today', '$new_savings_end')");
                }       


                if($sql_insert_savings){
                    $sql_wallet_details = $db->query("SELECT total_amount FROM debtor_wallets WHERE wallet_id='$wid'");
                
                    $total_amount = intval($sql_wallet_details->fetch_assoc()['total_amount']);

                    $balance = $total_amount + $calculated_amount;

                    $sql_update_wallet = $db->query("UPDATE debtor_wallets SET total_amount='$balance' WHERE wallet_id='$wid'");

                    if($sql_update_wallet){

                        $sql_check_for_completed = $db->query("SELECT * FROM debtor_wallets WHERE wallet_id='$wid'");
                        ...
                        if($total_amount >= $product_price){
                            $update_wallet_status = $db->query("UPDATE debtor_wallets SET completed='1' WHERE wallet_id='$wid'");
