<?php
    ...
    // RETURNS AN ARRAY OF DATES FROM $START_DATE TO $END_DATE
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($wid) || empty($daily_payment) || empty($days) || empty($pwd)){
            ...
            if(password_verify($pwd, $agent_passcode)){
                ...
                $sql_check_for_existing_savings = $db->query("SELECT * from agent_savings WHERE
                ...
                if($sql_check_for_existing_savings->num_rows > 0){
                    $sql_check_latest_savings = $db->query("SELECT * FROM agent_savings WHERE wallet_id='$wid' ORDER BY savings_id DESC"); 
                ...
                if($sql_insert_savings){
                    ...
                    if($sql_update_wallet){
                        ...
                        $sql_check_for_completed = $db->query("SELECT * FROM agent_wallets WHERE wallet_id='$wid'");
                        ...
                        if($total_amount >= $product_price){
                            $update_wallet_status = $db->query("UPDATE agent_wallets SET completed='1' WHERE wallet_id='$wid'");
